...
function formatDate($x){
   return strval(date("Y-m-d", strtotime($x)));
}
...
function checkInvalidData($nid,$name,$user,$pass,$db){
   $conn = oci_connect($user, $pass,$db);
   $locked='Y';
   $sql = 'SELECT count(*) FROM online_user.ekyc_fail_log WHERE (cust_nm=:cust_name or cust_nid=:cust_nid) and locked=:locked';
   $stid = oci_parse($conn, $sql);
   oci_bind_by_name($stid, ':cust_name', $name);
   oci_bind_by_name($stid, ':cust_nid', $nid);
   oci_bind_by_name($stid, ':locked', $locked);
   oci_execute($stid);
   $row = oci_fetch_array($stid, OCI_BOTH);
   $count= $row[0];
   if($count>=2){
